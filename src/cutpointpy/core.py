...
class CutpointCalculator():
    ...
    def __init__(self, target='youdenj', above=True, interpolation=None,
                 num_points=100):
        """
        Parameters
        ----------
        target : str
            The target function to maximise or minimise. Possible values:
                'youdenj' -> Maximum of Youden’s J (Sensitivity + Specificity 
                             - 1).
                'eucdist' -> Minimum of the Euclidean distance between the 
                             (0,1) point and the ROC curve in the FPR/TPR space.
        above : bool
            The direction of the inequality. If True the datapoints with 
            feature value above the threshold are flagged as positive, and 
            those with value below the threshold as negative. If False
            the reverse happens.
    ...
    def find(self, features, labels):
        ...
        References
        ----------
        1. Hassanzad and Haijan-Tilaki (2024) doi:10.1186/s12874-024-02198-2 
        """
        ...
        acc, se, sp, cutpoint, cutpoint_idx, auc = self._test_cutoff_values(
            features=features, 
            labels=labels, 
    ...
    def bootstrap(self, features, labels, method='sss', train_ratio=0.7, 
        ...
        aucs : ndarray of float (nruns, 1)
            The area under the curve for each bootstrap run estimated on the 
            train set.
        performance_train : ndarray of float (nruns, 3)
            In column-wise order, respectively accuracy, sensitivity and 
            specificity yielded by the optimal cut-point value when applied to 
            the train set.
        ...
        cutpoints, aucs =\
            [np.zeros(shape=(num_runs,1), dtype=float) for _ in range(2)]
        performance_train, performance_test, performance_whole =\
            [np.zeros(shape=(num_runs,3), dtype=float) for _ in range(3)]
        ...
        for split_idx, (train_idxs, test_idxs) in enumerate(
            ...
            #value estimated on the train set
            ...
            #Compute performance parameters on the whole dataset using the 
    ...
    def _compute_optimal_cutpoint(self, thresholds, se, sp):
        ...
        Note
        ----
        If the maximum of the target function (`self.target`) occurs on multiple 
        values of `thresholds`, the the first (smallest) occurrence of 
        `thresholds` is returned as the optimal cutpoint value. 
        """
